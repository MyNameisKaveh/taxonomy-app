...
# --- Helper Function: Wikipedia Image URL (Corrected try/except) ---
def get_wikipedia_image_url(species_name_from_user, scientific_name_from_gbif=None):
    ...
    # ... (بخش ساخت search_candidates و priority_keywords مثل قبل) ...
    ...
    while search_candidates:
        ...
        # --- ساختار try/except بازنویسی شده ---
        try: 
            wiki_page = None
            page_title_found = None
            
            try: # سعی در گرفتن مستقیم صفحه
                wiki_page = wikipedia.page(term_to_search, auto_suggest=True, redirect=True)
                page_title_found = wiki_page.title
                print(f"[WIKI_IMG] Found page directly: '{page_title_found}' for '{term_to_search}'")
            ...
            if wiki_page and wiki_page.images:
                print(f"[WIKI_IMG] Page '{page_title_found}' images (up to 5): {wiki_page.images[:5]}")
                ...
                print(f"[WIKI_IMG] No images listed on Wikipedia page: '{page_title_found}'")

        # --- بلوک except برای try اصلی ---
        except wikipedia.exceptions.PageError: 
             print(f"[WIKI_IMG] Wikipedia PageError (likely after search/disambiguation) for term: '{term_to_search}'")
             continue 
        except Exception as e:
            print(f"[WIKI_IMG] Generic error during processing for '{term_to_search}': {str(e)}")
            traceback.print_exc() 
            continue 
            
    print(f"[WIKI_IMG] No suitable Wikipedia image URL after all attempts.")
    return None
...
def main_handler(path=None):
    ...
    try:
        ...
        if data_gbif.get("matchType") == "EXACT" and data_gbif.get("confidence", 0) > 90 :
            ...
            classification_data.update({ # استفاده از update برای افزودن چندین کلید
                "scientificName": gbif_scientific_name_confirmed, "kingdom": data_gbif.get("kingdom"), "phylum": data_gbif.get("phylum"),
    ...
    # پاس دادن نام علمی (تایید شده یا ورودی) به تابع تصویر
    name_for_wiki_image = gbif_scientific_name_confirmed if gbif_scientific_name_confirmed else scientific_name_query
    # همچنین نام اولیه کاربر (که در اینجا همان نام علمی است) را هم پاس میدهیم
...
# --- اگر بخواهیم مستقیما فایل را اجرا کنیم (برای تست محلی) ---
# if __name__ == "__main__":
#     # برای اجرای محلی، نیاز به تعریف پورت و debug=True است
#     # این بخش روی Vercel اجرا نمی‌شود
