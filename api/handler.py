# api/handler.py (نسخه تک فایلی - اصلاح شده)

from flask import Flask, jsonify, request
import requests
import traceback
import wikipedia
from Bio import Entrez # ایمپورت BioPython اینجا
import time
...
ENTREZ_EMAIL = "andolini1889@gmail.com" # از ایمیل شما استفاده کردم
...
def get_best_ncbi_suggestion_flexible(common_name, max_ids_to_check=5):
    # ... (کد کامل و صحیح تابع get_best_ncbi_suggestion_flexible مثل قبل) ...
    ...
    try:
        ...
        summary_records = Entrez.read(summary_handle); summary_handle.close()            
...
def get_wikipedia_image_url(species_name_from_user, scientific_name_from_gbif=None):
     # ... (کد کامل و صحیح تابع get_wikipedia_image_url مثل قبل) ...
    search_candidates = []
    clean_scientific_name = None; clean_scientific_name_for_filename = None
    ...
    if user_name_for_filename:
        ...
        if "_" in user_name_for_filename:
            user_genus_equivalent = user_name_for_filename.split("_")[0]; add_user_genus = True
                ...
                for img_url in wiki_page.images:
                    ...
                    for pk_word in priority_keywords:
                        if pk_word in img_url_lower:
                            score += 5; filename_part = img_url_lower.split('/')[-1]
...
def suggest_name_endpoint():
    ...
    try:
        ...
        if suggestion: return make_response({"query": query, "scientific_name_suggestion": suggestion}, 200)
         ...
         print(f"Error in suggest_name_endpoint: {e}"); traceback.print_exc()
...
def main_handler(path=None):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if data_post: 
                ...
                # اگر بدنه JSON خالی بود یا None
                return jsonify({"error": "درخواست نامعتبر: بدنه JSON خالی است."}), 400, common_headers
        except Exception as e_post: 
            # اگر بدنه اصلا JSON نبود یا خطای دیگری در خواندنش بود
            print(f"Error parsing JSON body: {str(e_post)}\n{traceback.format_exc()}")
            return jsonify({"error": f"خطا در پردازش درخواست POST: {str(e_post)}"}), 400, common_headers
    ...
    try:
        ...
        if not data_gbif or data_gbif.get("matchType") == "NONE" or data_gbif.get("confidence", 0) < 30:
            ...
            classification_data.update({"matchType": data_gbif.get("matchType", "NONE"), "confidence": data_gbif.get("confidence")})
        else:
            gbif_scientific_name = data_gbif.get("scientificName")
            classification_data.update({"scientificName": gbif_scientific_name, "kingdom": data_gbif.get("kingdom"), "phylum": data_gbif.get("phylum"), "class": data_gbif.get("class"), "order": data_gbif.get("order"), "family": data_gbif.get("family"), "genus": data_gbif.get("genus"), "species": data_gbif.get("species") if data_gbif.get("speciesKey") and data_gbif.get("species") else None, "usageKey": data_gbif.get("usageKey"), "confidence": data_gbif.get("confidence"), "matchType": data_gbif.get("matchType"), "status": data_gbif.get("status"), "rank": data_gbif.get("rank")})   
