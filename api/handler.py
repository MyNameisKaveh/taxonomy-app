...
# --- تابع گرفتن تصویر از ویکی‌پدیا (اصلاح شده برای SyntaxError) ---
def get_wikipedia_image_url(species_name_from_user, scientific_name_from_gbif=None):
    ...
    if clean_scientific_name_for_filename and "_" in clean_scientific_name_for_filename: priority_keywords.append(clean_scientific_name_for_filename.split("_")[0]) # Fixed variable name
    ...
    while search_candidates:
        ...
        try: # --- شروع بلاک try اصلی برای هر term_to_search ---
            wiki_page=None
            try: # --- شروع try داخلی برای گرفتن صفحه ---
                wiki_page=wikipedia.page(term_to_search,auto_suggest=True,redirect=True); print(f"[WIKI_IMG] Found page directly: '{wiki_page.title}' for '{term_to_search}'")
            ...
            # --- پایان try داخلی برای گرفتن صفحه ---

            if wiki_page and wiki_page.images:
                print(f"[WIKI_IMG] Page '{wiki_page.title}' images (up to 5): {wiki_page.images[:5]}"); candidate_images_with_scores=[]
                for img_url in wiki_page.images:
                    ...
                    for pk_word in priority_keywords:
                        ...
                        # Fixed variable name in the next line
                ...
                if sorted_images and sorted_images[0]['score'] > 0:
                    ...
                    if best_image_url.startswith("//"): best_image_url="https:" + best_image_url # Fixed : addition
...
def suggest_name_endpoint():
    common_headers = {'Access-Control-Allow-Origin': '*'} # برای CORS
    query = request.args.get('query')
    if not query:
        return jsonify({"error": "Query parameter is required"}), 400, common_headers

    name_to_search_en, original_lang = translate_to_english_if_needed(query)
    suggestion = None
    if name_to_search_en: # فقط اگر نامی برای جستجو داشتیم
        suggestion = get_best_ncbi_suggestion_flexible(name_to_search_en)

    response_data = {
        "query": query,
        "searched_term_en": name_to_search_en,
    ...
    if suggestion:
        ...
        # حتی اگر پیشنهادی نبود، کد 200 برمیگردونیم چون خود عملیات موفق بوده
        return jsonify(response_data), 200, common_headers


# --- تابع اصلی جستجوی طبقه‌بندی (main_handler) ---
...
def main_handler(path=None):
    ...
    if request.method == 'GET': species_name_query_original = request.args.get('name')
        ...
        try:
            ...
            if data_post: species_name_query_original = data_post.get('name')
    ...
    if not species_name_query_original: return jsonify({"error": "Parameter 'name' required"}), 400, common_headers

    species_name_for_gbif = species_name_query_original
    ...
    if gbif_error_message and not final_data.get("scientificName"):
        ...
        # اگر فقط پیام خطا بود، 404 برمی‌گردانیم
...
if __name__ == "__main__":
    # این بخش فقط برای تست محلی است و روی Vercel اجرا نمی‌شود
    # برای تست محلی، مطمئن شوید که پورت دیگری استفاده می‌کنید اگر لازم است
