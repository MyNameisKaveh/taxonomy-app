...
import traceback
import wikipedia
from Bio import Entrez # فقط Entrez لازم است
import time
...
# --- تابع جستجوی NCBI (نسخه نهایی و خوب قبلی) ---
def get_best_ncbi_suggestion_flexible(common_name_en, max_ids_to_check=5):
    ...
    try:
        # فقط با خود نام جستجو می‌کنیم چون انتظار ورودی انگلیسی داریم
        ...
        for summary_record in summary_records:
            ...
            # print(f"    - Candidate TaxID {tax_id_processed}: '{sci_name}', Rank: {rank_ncbi}") # لاگ کمتر
            if sci_name:
...
# --- Endpoint جدید برای راهنما (فقط ورودی انگلیسی) ---
...
# --- توابع اصلی جستجو و تصویر (مانند قبل، بدون تغییر عمده) ---

# ... (کد تابع get_wikipedia_image_url اینجا قرار می‌گیرد - همان نسخه کامل قبلی) ...
def get_wikipedia_image_url(species_name_from_user, scientific_name_from_gbif=None):
        # ... (کد کامل و دقیق شده قبلی) ...
...
def main_handler(path=None):
    ...
    # این endpoint فقط نام علمی دقیق را از کاربر می‌گیرد
    scientific_name_query = ""
    if request.method == 'GET':
        scientific_name_query = request.args.get('name')
    elif request.method == 'POST':
        try:
            data_post = request.get_json()
            if data_post: scientific_name_query = data_post.get('name')
            else: return jsonify({"error": "Request body must contain JSON with 'name' field."}), 400, common_headers
        except Exception as e_post:
            print(f"Error parsing JSON body: {str(e_post)}\n{traceback.format_exc()}")
            return jsonify({"error": f"Error processing JSON request: {str(e_post)}"}), 400, common_headers

    if not scientific_name_query:
        return jsonify({"error": "Parameter 'name' (exact scientific name) is required."}), 400, common_headers
    ...
    classification_data = {"searchedName": scientific_name_query} # نام جستجو شده همان نام علمی است
    gbif_error_message = None
    gbif_scientific_name_confirmed = None # نام علمی تایید شده توسط GBIF
    ...
    try:
        ...
        # برای این endpoint اصلی، انتظار تطابق دقیق‌تری از GBIF داریم
        if data_gbif.get("matchType") == "EXACT" and data_gbif.get("confidence", 0) > 90 :
            ...
            classification_data.update({
                "scientificName": gbif_scientific_name_confirmed,
             ...
             # حتی اگر تطابق دقیق نبود، classification_data را با اطلاعات موجود پر میکنیم
    ...
    # گرفتن تصویر از ویکی‌پدیا با استفاده از نام علمی (تایید شده یا ورودی)
    name_for_wiki_image = gbif_scientific_name_confirmed if gbif_scientific_name_confirmed else scientific_name_query
    wiki_image_url = get_wikipedia_image_url(scientific_name_query, name_for_wiki_image) # ارسال نام علمی به عنوان اولویت
    
    if wiki_image_url:
        classification_data["imageUrl"] = wiki_image_url
    
    # اضافه کردن پیام خطا/هشدار GBIF به خروجی اگر وجود داشت و موفقیت کامل نبود
    if gbif_error_message and not gbif_scientific_name_confirmed:
        classification_data["gbifLookupMessage"] = gbif_error_message

    final_data = {k: v for k, v in classification_data.items() if v is not None}

    # همیشه با کد 200 برمیگردانیم مگر اینکه خطای اساسی رخ داده باشد
    # وجود یا عدم وجود اطلاعات کامل در خود final_data مشخص می‌شود
