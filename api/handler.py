# api/handler.py (نسخه تک فایلی برای تست)
...
def get_best_ncbi_suggestion_flexible(common_name, max_ids_to_check=5):
    # ... (کد کامل و صحیح تابع get_best_ncbi_suggestion_flexible که در utils.py بود) ...
...
def get_wikipedia_image_url(species_name_from_user, scientific_name_from_gbif=None):
    # ... (کد کامل و صحیح تابع get_wikipedia_image_url که قبلا داشتیم) ...
    search_candidates = []
...
def main_handler(path=None):
    ...
    # ... (کد کامل تابع main_handler مثل نسخه قبلی که کار می‌کرد) ...
    species_name_query = ""
    ...
    try:
        ...
        if not data_gbif or data_gbif.get("matchType") == "NONE" or data_gbif.get("confidence", 0) < 30:
            gbif_error_message = f"موجودی با نام '{species_name_query}' در GBIF پیدا نشد..."; classification_data.update({"matchType": data_gbif.get("matchType", "NONE"), "confidence": data_gbif.get("confidence")})
        else:
            gbif_scientific_name = data_gbif.get("scientificName"); classification_data.update({"scientificName": gbif_scientific_name, "kingdom": data_gbif.get("kingdom"), "phylum": data_gbif.get("phylum"), "class": data_gbif.get("class"), "order": data_gbif.get("order"), "family": data_gbif.get("family"), "genus": data_gbif.get("genus"), "species": data_gbif.get("species") if data_gbif.get("speciesKey") and data_gbif.get("species") else None, "usageKey": data_gbif.get("usageKey"), "confidence": data_gbif.get("confidence"), "matchType": data_gbif.get("matchType"), "status": data_gbif.get("status"), "rank": data_gbif.get("rank")})   
